        ...
        <section class="row">
            ...
            <div v-for="comment in comments" :key="comment" class="card col-12 mt-3">
                <div class="card-header bg-white ">
                    <p class="btn btn-secondary mx-auto rounded p-2 buttonsPanel w-100" style="cursor:default">Commentaire rédigé le {{ comment.createdAt.slice(0,10).split('-').reverse().join('/') }}  par {{ comment.User.userName.charAt(0).toUpperCase() + comment.User.userName.slice(1) }}</p>
                </div>
                <div class="card-body text-right rounded mx-2">
                    <p>  {{ comment.comment }} </p>
                </div>
                <div class="card-footer row justify-content-between bg-white ">
                    <p class="btn btn-secondary mx-auto rounded p-2 buttonsPanel w-100 " style="cursor:default">ref# <span class="badge badge-light"> {{ comment.id }}</span></p>
